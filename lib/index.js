...
class ScrollableTabView extends Component {
  ...
  _onChangeTab(prevPage, currentPage) {
    ...
    if (this.contentScrollDistance >= this.collapsableBarHeight) {
      this.contentView.scrollTo({x: 0, y: this.collapsableBarHeight, animated: true})
    }
    if (this.props.collapsableBar) {
      const statusBarHeight = this.props.statusBarHeight || 0;
      const height = Dimensions.get('window').height - statusBarHeight + this.collapsableBarHeight;
      setTimeout(() => {
        if (currentPage === 0 && this.props.tab1Height) {
          const newHeight = this.collapsableBarHeight + this.props.tab1Height < height ? height : this.collapsableBarHeight + this.props.tab1Height + statusBarHeight + 30;
          this.setState({ height: newHeight })
        } else if (currentPage === 1 && this.props.tab2Height) {
          const newHeight = this.collapsableBarHeight + this.props.tab2Height < height ? height: this.collapsableBarHeight + this.props.tab2Height + statusBarHeight + 30 ;
          this.setState({ height: newHeight < height ? height : newHeight })
        } else if (currentPage === 2 && this.props.tab3Height) {
          const newHeight = this.collapsableBarHeight + this.props.tab3Height < height ? height : this.collapsableBarHeight + this.props.tab3Height + statusBarHeight + 30;
          this.setState({ height: newHeight < height ? height : newHeight })
        }
      }, 100) //250 works in IOS Dev Mode
    }
  }
  ...
  render() {
    ...
    const statusBarHeight = this.props.statusBarHeight || 0
    const height = isScrollView && this.state.height || isScrollView && Dimensions.get('window').height - statusBarHeight + this.collapsableBarHeight
    ...
    return (<ContainerView
        ...
        style={[styles.container, this.props.style, this.props.contentStyle]}
        ...
        contentContainerStyle={[this.props.collapsableBar && this.props.contentStyle, {height}] || {}}>
