...
class ScrollableTabView extends Component {
  ...
  componentDidMount() {
    setTimeout(() => {
      InteractionManager.runAfterInteractions(() => {
        if (Platform.OS === 'android' && this.props.initialPage !== 0) {
  ...
  componentWillReceiveProps(props) {
    ...
    const height = Dimensions.get('window').height - statusBarHeight + (this.collapsableBarHeight || 0);
    if (props.tabHeights && props.tabHeights[this.state.currentPage]) {
      const newHeight = this.collapsableBarHeight + props.tabHeights[this.state.currentPage] + 56 <= height ? height : this.collapsableBarHeight + props.tabHeights[this.state.currentPage] + 56;
  ...
  _onChangeTab(prevPage, currentPage) {
    ...
    if (this.props.collapsableBar) {
      ...
      setTimeout(() => {
        if (this.props.tabHeights && this.props.tabHeights[currentPage]) {
          const newHeight = this.collapsableBarHeight + this.props.tabHeights[currentPage] + 56 < height ? height : this.collapsableBarHeight + this.props.tabHeights[currentPage] + 56;
          this.setState({ height: newHeight })
        }
  ...
  render() {
    ...
    const statusBarHeight = this.props.statusBarHeight || 0
    const height = isScrollView ? this.state.height || Dimensions.get('window').height - statusBarHeight + (this.collapsableBarHeight || 0) : null;
